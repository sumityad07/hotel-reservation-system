    ...
    <>
      ...
      <div className='block sm:hidden flex flex-col items-center justify-center h-screen gap-4'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col items-center justify-center gap-4'>
          ...
          <div className='flex flex-col items-center justify-center text-center  gap-4'>
            <Input_Btn type="submit" disabled={loading} text={loading ? "Logging in..." : "loggin"} name="Login" />
            <Link to ="/signup">
            <p className=''>new here let Signup --{'>'}</p>
            </Link>
          </div>
      ...
      <div className='hidden md:block md:flex px-20 py-20 md:justify-evenly'>
        <div className="left_side flex flex-col justify-center items-start gap-8">
          ...
          <form onSubmit={handleSubmit} className='flex flex-col gap-4 justify-center'>
            ...
            <div className='flex justify-center items-center'>
              <Input_Btn type="submit" disabled={loading} text={loading ? "Signing Up..." : "Sign Up"} name ="Login" />
            </div>
          </form>
          {error && <p className="text-red-500 text-center">{error}</p>}
          {successMessage && <p className="text-green-500 text-center">{successMessage}</p>}
        ...
        <div className="right_side">
          <img className='w-[548px] h-auto' src={signupbtn_full} alt="login Illustration" />
        </div>
      </div>
    </>
  );
